...
$(function () {
    ...
    $('input[title], textarea[title], .prompt_title[title]').tooltip({
        position: {
            my: 'left+8 center'
        }
    });
    ...
    $(window).resize(function () {
        ...
        $('#login_container').css({
            'left': ($(window).width() - $('#login_container').width()) / 2
        });

        // 後台首頁
        $('#hcb_left, #hcb_right').css({
            'height': $(window).height() - 97
        });
        $('#hch_right, #hcb_right, #hcf_right').css({
    ...
    // 後台登錄頁
    $('#login_container').draggable({
        containment: '#login_body',
        scroll: false,
        handle: '#lc_title'
    });
    ...
    $('#power_form').submit(function () {
        ...
        if ((select_length == 3 && $('#power_url').val() == 'default') || (select_length != 3 && $('#power_url').val() != 'default')) {
            $('#power_url').focus();
            return false;
        }
        if ((select_length == 1 && $('input[name="power_site"]:checked').val() == 1) || (select_length != 1 && $('input[name="power_site"]:checked').val() != 1)) {
            $('#power_site_prompt').focus();
            return false;
        }
        if ((select_length == 3 && $('input[name="power_target"]:checked').val() == 1) || (select_length != 3 && $('input[name="power_target"]:checked').val() != 1)) {
            $('#power_target_prompt').focus();
            return false;
        }
        if (select_length != 1 && $('#power_ico').val()) {
            $('#power_ico').focus();
            return false;
        }
    });
    $('#group_power_form').submit(function () {
        ...
        if ($('input[name="power_ids[]"]:checked').length == 0) {
            show_dialog('請選擇一項權限');
            return false;
        }
    });
            ...
            '刪除': function () {
                if (action_url) {
                    ...
                    $.get(action_url, function (msg) {
                        if (msg == 1) {
                            show_dialog('刪除成功');
                            $('#' + action_obj).fadeOut(2000, function () {
                                $(this).remove();
                            });
                        } else {
