...
function stepless_classification(obj_id, select_name, ajax_url, pid, id_str, display_type, option_default, select_level) {
	...
	if (isNaN(parseInt(pid))) {
		...
		ajax(ajax_url + pid + (id_str ? id_str : (option_default ? option_default : '')) + (display_type ? display_type : ''), back_stepless_classification, '', {
			obj_id: obj_id,
...
function back_stepless_classification(ajax, json)
{
	...
	var chk_last = eval(rt);
	
	// 最後一層後 不要在往下找
	if (json.select_level == '3')
		return ;
	...
	//最後一層時秀出
	// if (json.select_level == '2') //0->1->2
	// {
	// 	sel.options[0] = new Option('請增加', '');
	// 	sel.disabled = true;
	// }	
	// else	
